...
exports.WebhookService = {
    handleWebhookNotification: (webhookId) => __awaiter(void 0, void 0, void 0, function* () {
        // 1. Find subscription
        // Note: In a real high-scale app, we might check all subscriptions with this ID, 
        // but webhook ID is unique per base/install.
        const subscription = yield webhook_model_1.WebhookSubscription.findOne({ webhookId });
        if (!subscription) {
        ...
        try {
            const data = yield airtable_service_1.AirtableService.getWebhookPayloads(user, subscription.baseId, webhookId, subscription.cursor);
                    ...
                    for (const tableId in payload.changedTablesById) {
                        ...
                        if (tableChanges.changedRecordsById) {
                            for (const recordId in tableChanges.changedRecordsById) {
                                ...
                                const submission = yield submission_model_1.Submission.findOne({ airtableRecordId: recordId });
                                if (submission) {
                        ...
                        if (tableChanges.destroyedRecordIds) {
                            for (const recordId of tableChanges.destroyedRecordIds) {
                                yield submission_model_1.Submission.updateOne({ airtableRecordId: recordId }, { $set: { deletedInAirtable: true } });
